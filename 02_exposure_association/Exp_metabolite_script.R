...
samples <- rownames(metabolites)
covar_exp_merge <- filter(covar_exp_merge, subjectidp %in% samples)
metabolites <- filter(metabolites, rownames(metabolites) %in% covar_exp_merge$subjectidp)
...
icc_diffs <- rep(0, ncol(metabolites))     # Create item to store differences in ICCs
names(icc_diffs) <- colnames(metabolites)

fixed_eff <- paste0(c('age', 'gender', 'bmi'), collapse = '+')     # List of fixed effects
rand_eff_list <- c('(1 | id)')     # List of random effects (extracted in mixed models)
rand_eff <- paste0(rand_eff_list, collapse = '+')

for (column in 1:ncol(metabolites)) {
  y <- paste0('metabolites[, ', column, '] ~ ')
...
temp <- rep(0,ncol(metabolites))     # temp vector to store 'null' ICC differences
...
rownames(null_icc_diff_matrix) <- colnames(metabolites)
...
exp_diff_meta <- rep(0, length(icc_diffs))
names(exp_diff_meta) <- names(icc_diffs)

for (column in 1:length(icc_diffs)) {
  exp_diff_meta[column] = sum(null_icc_diff_matrix[column,] >= icc_diffs[column])/length(null_icc_diff_matrix[column,])
}


###### Save p-values ######
ifelse(dir.exists("Results"),"",dir.create("Results", showWarnings = FALSE))
saveRDS(exp_diff_meta, file = 'Results/exp_diff_meta.rds')
