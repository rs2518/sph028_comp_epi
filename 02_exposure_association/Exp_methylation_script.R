...
for (i in 1:ncol(methylation)) {
  if ( (sum(is.na(methylation[,i]))/length(methylation[,i])) > 0.95) {
...
s = sample(ncol(methylation), size = 10)
...
samples <- rownames(methylation)
covar_exp_merge <- filter(covar_exp_merge, subjectidp %in% samples)
methylation <- filter(methylation, rownames(methylation) %in% covar_exp_merge$subjectidp)
...
icc_diffs <- rep(0, ncol(methylation))     # Create item to store differences in ICCs
names(icc_diffs) <- colnames(methylation)

fixed_eff <- paste0(c('age', 'gender', 'bmi'), collapse = '+')     # List of fixed effects
rand_eff_list <- c('(1 | id)', '(1 | chip)', '(1 | position)')     # List of random effects (extracted in mixed models)
rand_eff <- paste0(rand_eff_list, collapse = '+')

for (column in 1:ncol(methylation)) {
  y <- paste0('methylation[, ', column, '] ~ ')
...
temp <- rep(0,ncol(methylation))     # temp vector to store 'null' ICC differences
...
rownames(null_icc_diff_matrix) <- colnames(methylation)
...
exp_diff_methyl <- rep(0, length(icc_diffs))
names(exp_diff_methyl) <- names(icc_diffs)

for (column in 1:length(icc_diffs)) {
  exp_diff_methyl[column] = sum(null_icc_diff_matrix[column,] >= icc_diffs[column])/length(null_icc_diff_matrix[column,])
}


###### Save p-values ######
ifelse(dir.exists("Results"),"",dir.create("Results", showWarnings = FALSE))
saveRDS(exp_diff_methyl, file = 'Results/exp_diff_methyl.rds')
